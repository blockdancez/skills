...
12. 如果表单出现验证码或机器人验证，不要立刻停止。先把项目资料和公开提交信息能够确定的所有必填、可选字段全部填好；需要图片时默认上传当前项目资料夹中的 `screenshot.png`，只有平台明确要求 logo 时才改用 `logo.png`。然后按用户授权尽可能完成验证。验证码不可读、无法自动完成、需要外部设备/账号确认或需要用户人工判断时，才设为 `needs_user`，保留已经填好的表单标签页，让用户只需完成验证并提交。
...
- 只有验证码无法代填或需要用户人工处理时，才把当前项设为 `needs_user`；设置前必须先填完所有可由项目资料和公开提交信息确定的字段，并默认上传当前项目的 `screenshot.png`。如果验证码已完成，继续提交当前表单。
...
验证码不是默认停止条件。遇到 CAPTCHA、验证码或机器人验证时，先填完所有可确定的表单字段并默认上传项目 `screenshot.png`，再尽可能帮用户完成验证；只有无法自动完成、不可读、需要外部设备/账号确认或需要用户人工判断时，才停止并设为 `needs_user`。保留的页面必须处于“用户只需完成验证码/人机验证并提交”的状态，不能留下仍可由项目资料补齐的空字段。
