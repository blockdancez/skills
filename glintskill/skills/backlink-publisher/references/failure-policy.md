...
- CAPTCHA、验证码或机器人验证无法代填，或者需要外部设备/账号确认、用户人工判断。
...
验证码、CAPTCHA 或机器人验证触发 `needs_user` 前，必须先填完所有能由项目资料和公开提交信息确定的字段。图片字段默认上传当前项目的 `screenshot.png`；只有字段明确要求 logo/icon 时才使用 `logo.png`。保留的标签页应让用户只需处理验证并提交，不能留下仍可自动补齐的空字段。
