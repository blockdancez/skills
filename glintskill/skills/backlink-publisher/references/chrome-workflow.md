...
8. 只上传所选项目资料夹中的项目资产。
9. 如果出现 CAPTCHA、验证码或机器人验证，按用户授权尽可能帮用户代填或完成验证。不要绕过验证；如果验证码不可读、无法自动完成、需要外部设备/账号确认或需要用户人工判断，按失败处理策略设为 `needs_user` 并保留标签页。
...
用户授权尽可能代填免费 listing 流程中的验证码和所有可由公开资料确定的表单字段；该授权不覆盖绕过验证、使用私密信息、访问外部设备、读取验证码短信/邮件或执行 listing 之外的权限授权。
