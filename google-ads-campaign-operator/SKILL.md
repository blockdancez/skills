...
- **Keyword Planner / keyword research**: read `references/prelaunch-and-keyword-research.md`; explain whether to use `Discover new keywords` or `Get search volume and forecasts`, what fields to fill, what to export, and how to decide seed, exact, phrase, negative, and rejected terms.
...
- **First campaign creation**: read `references/workflow-router-and-intake.md`, then Campaign Setup, Ad Group Setup, First Responsive Search Ad, URL Options, and Conversion Setup in `references/google-ads-practical-playbook.md`.
- **Beginner / first launch**: read Beginner Wizard Mode, Pre-launch Checklist, and Post-launch Review Cadence in `references/google-ads-practical-playbook.md`, plus readiness rules in `references/prelaunch-and-keyword-research.md`; output a complete fill-in-the-screen workflow.
